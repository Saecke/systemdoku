...
### 17 — [Update-Manager]
Automatisiertes Server-Update-System auf dem i3 (Ubuntu).
- **steamcmd.py:** Steam-API-Polling → Build-ID-Vergleich → Discord-Webhook
- **pp_updater.py:** Selenium gegen PP-Panel → Stop → Update → Start (TODO)
- **Ingame-Warnungen:** #announce über SMB → gamechat/ auf Windows
- **Ablauf:** Update erkennen → Spieler warnen → Save → Stop → Update → Start → Discord-Meldung
- **Keine Abhängigkeit** zum Windows-Bot - komplett eigenständig, Dorf-Prinzip
...
- **Ladezustand:** 43 aktive, 3 deaktivierte, 4 nicht eingebundene Cogs (Stand Maerz 2026)
...
| AntiSpam | antispam.py | Spam-Erkennung → Mute/Ban (Duplikate, Flood, Cross-Channel, Mentions) | [16](16_COGS_REFERENZ.md) |
